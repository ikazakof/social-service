...
public class FriendBirthdayNotificationHandler implements NotificationHandler {
    ...
    public List<UserNotification> fillReceiverIds(UserNotification userNotification) {
        ...
        List<UserNotification> notificationList = friendsIds.stream().filter(id -> settingsRepository.findByUserId(id).orElseThrow().getFriendBirthday()).map(id -> {
